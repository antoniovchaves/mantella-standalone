...
export function MessageBubble({ message }: Props) {
  const { type, content, sender, timestamp, action } = message;
  ...
  if (type === 'player') {
    return (
      <div className="flex flex-col items-end gap-1 mb-3">
        ...
        <span className="text-[10px] text-stone-600 mr-1">{formatTime(timestamp)}</span>
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col items-start gap-1 mb-3">
      ...
      <span className="text-[10px] text-stone-600 ml-1">{formatTime(timestamp)}</span>
    </div>
  );
}
