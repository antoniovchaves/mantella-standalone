...
export function OverrideNPCPanel({ disabled, onAddNPC }: Props) {
  ...
  const loadNPCs = useCallback(async () => {
    ...
    try {
      ...
      setError('Proxy offline ou caminho inválido.');
      setPathExists(false);
    } finally {
      setLoading(false);
    }
  ...
  async function handleSetPath() {
    try {
      ...
      setError('Não foi possível atualizar o caminho.');
    }
  }

  // Agrupar por arquivo de origem
  const byFile = overrideNPCs.reduce<Record<string, OverrideNPC[]>>((acc, npc) => {
    const key = npc._source_file || 'sem arquivo';
    if (!acc[key]) acc[key] = [];
    acc[key].push(npc);
    return acc;
  }, {});
  ...
  return (
    <section>
      ...
      >
        <span>NPCs do Mantella {hasNPCs && `(${overrideNPCs.length})`}</span>
        ...
        <div className="flex flex-col gap-3">
          ...
          <div className="bg-stone-900 border border-stone-800 rounded-lg p-2.5">
            <div className="flex items-center justify-between mb-1">
              <span className="text-[10px] text-stone-600 uppercase tracking-widest">Pasta de override</span>
              <button
                onClick={() => { setEditingPath(true); setTempPath(overridePath); }}
            ...
            {editingPath ? (
              ...
              <div className="flex items-center gap-1.5">
                <div className={`w-1.5 h-1.5 rounded-full flex-shrink-0 ${
                  pathExists === null ? 'bg-stone-600' :
                  pathExists ? 'bg-emerald-500' : 'bg-red-500'
                }`} />
                <span className="text-[10px] text-stone-500 truncate font-mono" title={overridePath}>
                  {overridePath
                    ? overridePath.replace(/\\/g, '/').split('/').slice(-3).join('/')
                    : 'não configurado'}
                </span>
              </div>
            )}
            ...
            {pathExists === false && !editingPath && (
              <p className="text-[10px] text-red-500/70 mt-1.5">
                Pasta não encontrada. O caminho padrão é:<br />
                <span className="font-mono">Documentos\My Games\Mantella\data\Skyrim\character_overrides</span>
              </p>
            )}
          </div>
          ...
          >
            {loading ? 'Carregando...' : `↺ Recarregar arquivos`}
          </button>

          {error && (
            <p className="text-[11px] text-red-500/70">{error}</p>
          )}

          {/* Lista de NPCs por arquivo */}
          {!loading && hasNPCs && Object.entries(byFile).map(([file, npcs]) => (
            <div key={file}>
              <div className="text-[10px] text-stone-600 font-mono mb-1.5 truncate" title={file}>
                📄 {file}
              </div>
              <div className="flex flex-col gap-1.5">
                {npcs.map((npc, i) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="text-xs font-medium text-stone-200 truncate">{npc.name}</div>
          ...
          {!loading && !hasNPCs && pathExists && (
            <p className="text-[11px] text-stone-600 italic">
              Nenhum NPC encontrado nos arquivos de override.<br />
              Adicione arquivos .json ou .csv na pasta configurada.
            </p>
          )}
          ...
          {!hasNPCs && (
            <details className="text-[10px] text-stone-600">
              ...
              <pre className="mt-2 bg-stone-900 border border-stone-800 rounded p-2 text-emerald-700/80 leading-relaxed overflow-x-auto">{`// meu_npc.json
{
  "name": "Aelindra",
  "race": "Dunmer",
  "gender": "female",
  "bio": "Maga exilada de Morrowind...",
  ...
  "base_id": "0x00000043"
              ...
              <p className="mt-1.5">
                ...
                <span className="font-mono text-stone-500">
                  Documentos\My Games\Mantella\data\Skyrim\character_overrides\
                </span>
              </p>
            </details>
          )}
