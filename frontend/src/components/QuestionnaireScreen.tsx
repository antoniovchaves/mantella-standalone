    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col gap-1.5">
        ...
        <div className="flex justify-between text-[10px] text-stone-500">
          ...
          <span className={`font-semibold text-xs transition-colors ${value != null ? 'text-amber-400' : 'text-stone-600'}`}>
            {value != null ? value : '—'}
          </span>
        ...
        <div className="flex justify-between px-[2px]">
          {POINTS.map((p) => (
            <span
              key={p}
              className={`text-[9px] w-4 text-center transition-colors ${
                value === p ? 'text-amber-500' : 'text-stone-700'
              }`}
            >
              {p}
            </span>
...
export function QuestionnaireScreen({ result, onDone }: Props) {
  ...
  const [exported, setExported] = useState<null | 'json' | 'csv' | 'both'>(null);
        ...
        <div className="w-full max-w-lg flex flex-col items-center gap-8 text-center">
          ...
          <div>
            ...
            <p className="text-sm text-stone-500">
              Obrigado pela sua participação. Exporte os resultados abaixo.
            </p>
          </div>
          ...
          <div className="w-full bg-stone-900 border border-stone-800 rounded-xl p-5 flex flex-col gap-3">
            ...
            {QUESTIONNAIRE.map((q) => (
              <div key={q.id} className="flex items-center justify-between gap-3">
                <span className="text-xs text-stone-400 flex-1 leading-relaxed">{q.text}</span>
                <div className="flex items-center gap-2 flex-shrink-0">
                  ...
                  <div className="w-20 h-1.5 bg-stone-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${((answers[q.id] - 1) / (SCALE - 1)) * 100}%` }}
                    />
                  </div>
                  <span className="text-xs font-semibold text-amber-400 w-4 text-right">
                    {answers[q.id]}
                  </span>
                </div>
              </div>
            ...
            <div className="pt-2 border-t border-stone-800 flex justify-between text-xs text-stone-500">
              ...
              <span className="text-stone-300 font-semibold">
                {(Object.values(answers).reduce((a, b) => a + b, 0) / QUESTIONNAIRE.length).toFixed(2)}
              </span>
            </div>
          </div>
          ...
          <div className="w-full flex flex-col gap-3">
            <p className="text-xs text-stone-600">Exportar resultados</p>
      ...
      <div className="w-full max-w-2xl flex flex-col gap-8">
        ...
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">
            Indique sua concordância em cada afirmação abaixo (escala de 1 a {SCALE}).
          </p>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          <div className="flex justify-between text-xs text-stone-600">
            <span>{Object.keys(answers).length} de {QUESTIONNAIRE.length} respondidas</span>
          ...
          {QUESTIONNAIRE.map((q, i) => (
            ...
            >
              <div className="flex items-center gap-2 mb-4">
                <span className="text-xs font-semibold text-stone-600 w-5">{i + 1}.</span>
                {answers[q.id] != null && (
        ...
        >
          {allAnswered ? 'Enviar respostas' : `Responda todas as ${QUESTIONNAIRE.length} perguntas para continuar`}
        </button>
